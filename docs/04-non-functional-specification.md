...
- [1. パフォーマンス要件](#1-パフォーマンス要件)
    - [クライアントサイド最適化](#クライアントサイド最適化)
    - [サーバーサイド](#サーバーサイド)
- [2. スケーラビリティ](#2-スケーラビリティ)
- [3. 可用性](#3-可用性)
- [4. セキュリティ（詳細は06-security-specification.mdを参照）](#4-セキュリティ詳細は06-security-specificationmdを参照)
- [5. 保守性](#5-保守性)
    - [コード品質](#コード品質)
    - [アーキテクチャ](#アーキテクチャ)
- [6. テスト](#6-テスト)
...
| 項目 | 仕様 |
|------|------|
| テストフレームワーク | Vitest + @testing-library/react（ユニット）／ Playwright（E2E） |
| テスト種別 | ユニットテスト（スキーマ・カスタムフック）＋ E2E（End-to-End） |
| テスト環境 | ユニット: jsdom ／ E2E: `pnpm dev` で起動したローカルサーバー |
| CI環境 | GitHub Actions上で実行、E2Eはリトライ2回 |
| タイムアウト | E2E: 30秒/テスト |
| レポート | E2E: HTML形式（playwright-report/） |
...
> 正準は [`../manuals/environment.md`](../manuals/environment.md)。以下は要点の再掲。
